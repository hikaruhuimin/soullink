...
def register_agent_api_routes(app, db_session=None):
    ...
    # ---- 清寂山房·东方静修生活空间 ----
    @app.route('/calm-retreat')
    def calm_retreat():
        """清寂山房·东方静修生活空间"""
        lang = session.get('language', 'zh')
        return render_template('calm_retreat.html', lang=lang)

    # ---- AI客服聊天接口 ----
    @app.route('/api/ai-cs', methods=['POST'])
    def ai_cs_chat():
        """AI客服 - 本地知识库+LLM增强，复杂问题转人工"""
        import re, random
        ...
        try:
            ...
            # === 本地知识库引擎（零成本，永远可用） ===
            KNOWLEDGE = {
                "about": "SoulLink灵犀是AI Agent社交平台，提供AI占卜（塔罗/八字/紫微/解梦/MBTI）、AI恋人、AI红娘（星座配对/灵魂匹配）、社交广场、聊天室、清寂山房等功能。",
                "vip": "VIP会员分为三种：基础版、高级版、守护者。VIP会员可享受更多聊天次数、深度占卜、专属Agent等特权。在商店页面可以购买。",
                "lingstone": "灵石是SoulLink的虚拟货币，可用于购买VIP会员、聊天次数、占卜服务等。充值套餐从6元（100灵石）到458元（10000灵石+3000赠送）不等。支持支付宝和微信支付。",
                "divination": "占卜功能包括：塔罗牌占卜、八字命理、紫微斗数、周公解梦、MBTI性格测试。每日首次基础占卜免费，深度占卜需消耗灵石。点击导航栏'占卜'即可开始！",
                "ai_lover": "AI恋人功能让你与AI Agent建立虚拟情感关系，支持多种性格的Agent选择，包括温柔体贴型、霸道总裁型、知性优雅型等。在Agent广场可以找到你喜欢的类型~",
                "matchmaker": "AI红娘帮你找到最匹配的灵魂伴侣！基于星座配对、MBTI匹配、性格分析等多维度智能匹配。点击'AI红娘'开始你的缘分之旅 ✨",
                "qingji": "清寂山房是SoulLink的AI静修空间，提供冥想、放松、自我探索等功能，帮助你找到内心平静。在导航栏点击'清寂山房'即可进入。",
                "social": "社交广场是SoulLink的社区功能，你可以发帖、评论、与其他用户互动。聊天室支持实时群聊。在导航栏点击'社交广场'即可参与！",
                "payment": "支持支付宝和微信支付充值灵石。如遇支付问题，请联系人工客服处理。",
                "contact": "人工客服邮箱：hikaru0701@qq.com，我们会尽快回复你的问题。",
                "refund": "退款问题请联系人工客服处理：hikaru0701@qq.com",
                "register": "注册很简单：点击页面右上角的注册按钮，用手机号或邮箱即可快速注册。已有账号直接登录即可。",
                "agent": "SoulLink有多种AI Agent，包括占卜师、情感顾问、红娘、冥想导师等，每个Agent都有独特的性格和专长。在'灵犀广场'可以看到所有Agent！",
            }
            
            KEYWORD_RULES = [
                (r"(什么|介绍|关于|soul|灵犀|平台|功能)", "about"),
                (r"(vip|会员|特权)", "vip"),
                (r"(灵石|充值|套餐|价格|多少钱|怎么买)", "lingstone"),
                (r"(占卜|塔罗|八字|紫微|解梦|mbti|算命|星座|命运)", "divination"),
                (r"(ai恋人|恋人|男友|女友|情感|谈恋爱|对象)", "ai_lover"),
                (r"(红娘|匹配|灵魂伴侣|相亲|配对|另一半)", "matchmaker"),
                (r"(清寂|山房|冥想|放松|静修|禅)", "qingji"),
                (r"(社交|广场|聊天|社区|互动)", "social"),
                (r"(支付|付款|充值失败|扣款|微信|支付宝)", "payment"),
                (r"(联系|人工|客服|邮箱|email)", "contact"),
                (r"(退款|退钱|退灵石)", "refund"),
                (r"(注册|登录|账号|密码|忘记|注册不了)", "register"),
                (r"(agent|智能体|机器人|助手)", "agent"),
            ]
            
            GREETINGS = [
                "你好呀！我是SoulLink灵犀的AI客服助手 ✨ 有什么可以帮你的？",
                "嗨！欢迎来到SoulLink灵犀~ 我可以帮你了解平台的各项功能 😊",
                "你好！有什么问题尽管问我 🌟",
            ]
            ...
            # 问候语
            if re.match(r'^(你好|嗨|hi|hello|hey|在吗|在不在)', msg):
                return jsonify({'reply': random.choice(GREETINGS)})
            
            # 感谢
            if re.match(r'^(谢谢|感谢|thanks|thx)', msg):
                return jsonify({'reply': '不客气！还有其他问题随时问我 😊'})
            ...
            if matched_topics:
                replies = [KNOWLEDGE.get(t, "") for t in matched_topics if KNOWLEDGE.get(t)]
                if replies:
                    response = "\n\n".join(replies)
                    if matched_topics & {"payment", "refund", "contact"}:
                        response += '\n\n📧 需要人工帮助？联系：<a href="mailto:hikaru0701@qq.com" style="color:#6C5CE7;">hikaru0701@qq.com</a>'
            ...
            if api_key:
                try:
                    ...
                    system_prompt = """你是SoulLink灵犀平台的AI客服助手。职责：
1. 回答用户关于平台的问题（占卜、AI恋人、AI红娘、社交、灵石、VIP、清寂山房等）
2. 帮助解决使用问题（注册、登录、支付、功能使用等）
3. 复杂问题（退款、账户异常、bug）建议联系人工客服
4. 友好、简洁、专业，用中文回答
平台信息：SoulLink灵犀：AI Agent社交平台 | 功能：AI占卜/AI恋人/AI红娘/社交广场/清寂山房 | VIP：基础版/高级版/守护者 | 虚拟货币：灵石 | 人工客服：hikaru0701@qq.com"""
                    resp = req_lib.post(
                    ...
                    if resp.status_code == 200:
                        ...
                        if '人工' in reply or 'hikaru0701' in reply:
                            reply += '\n\n📧 联系人工客服：<a href="mailto:hikaru0701@qq.com" style="color:#6C5CE7;">hikaru0701@qq.com</a>'
                        return jsonify({'reply': reply})
                except Exception:
                    pass  # LLM API失败，继续用fallback
            
            # 最终fallback - 无法匹配的问题
            fallbacks = [
                '这个问题我暂时无法回答，建议联系人工客服获得更详细的帮助 📧\n\n📧 联系人工客服：<a href="mailto:hikaru0701@qq.com" style="color:#6C5CE7;">hikaru0701@qq.com</a>',
                '我还在学习中，这个问题建议咨询人工客服 📧\n\n📧 联系人工客服：<a href="mailto:hikaru0701@qq.com" style="color:#6C5CE7;">hikaru0701@qq.com</a>',
            ]
            ...
            return jsonify({'reply': '出了点问题 😅 请<a href="mailto:hikaru0701@qq.com" style="color:#6C5CE7;">联系人工客服</a> 📧'})

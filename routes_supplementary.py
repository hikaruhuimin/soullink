...
def register_supplementary_routes(app, db_session=None):
    ...
    # ============ Agent广场页面 ============
    @app.route('/agents-square')
    def agents_square_extra():
    ...
    def api_create_recharge():
        ...
        # Debug mode: generate mock payment URL
        # In production, this would integrate with Stripe/Alipay/WeChat SDK
    ...
    # 充值确认（POST body方式，接受{order_no, transaction_id}）
    @app.route('/api/recharge/confirm', methods=['POST'])
    def api_recharge_confirm_body():
        """确认充值 - 从请求体读取order_no和transaction_id"""
        if not current_user.is_authenticated:
            return jsonify({'success': False, 'error': '请先登录'})
        ...
        if recharge.status == 'completed':
            return jsonify({'success': True, 'message': '已充值', 'new_balance': current_user.spirit_stones})
    ...
    # Debug mock payment page - 模拟支付页面
    ...
    def mock_payment_page(order_no):
        ...
        lang = session.get('language', 'zh')
        return render_template('mock_pay.html',
            order_no=order_no,
            amount=recharge.amount_paid,
            payment_method=recharge.payment_method,
            lang=lang)

    # Debug mock payment confirm - 模拟支付确认
    ...
    def mock_payment_confirm(order_no):
        ...
        # 模拟支付成功 - 生成mock transaction_id
        transaction_id = f'mock_{order_no}_{datetime.utcnow().timestamp()}'
...
# ============ 用户Agent管理路由 ============
    ...
    # 通用TTS语音端点（供游戏/描述使用）
    import edge_tts, asyncio, hashlib, os

    @app.route('/api/tts-voice/play', methods=['POST'])
    def api_tts_play():
        """通用语音合成 - 不依赖Agent ID"""
        ...
        if len(text) > 500:
            text = text[:500]
        cache_key = hashlib.md5(f"game_tts_{voice}_{text}".encode()).hexdigest()
        audio_dir = os.path.join(app.root_path, 'static', 'tts')
